...
// 规格选项列表
export const sku_card_list:any= ref([])

export const sku_list:any = ref([])
...
export function sortCard(action:any, index:any) {
    ...
    func(oList, index)
    let sortData = oList.map((o:any, i:any) => {
        return {
            id: o.id,
            order: i + 1
        }
...
export function handleChooseSetGoodsSkusCard(id:any,data:any){
    ...
    .then(res=>{
        console.log(res);
        item.name = item.text = (res as any).goods_skus_card.name
        item.goodsSkusCardValue = (res as any).goods_skus_card_value.map((o:any)=>{
            o.text = o.value || "属性值"
            return o
        })
        getTableData()
...
// 初始化规格的值
export function initSkusCardItem(id:any) {
