...
import { ref } from "vue"
import ImageAside from "@/components/publicComponents/ImageAside.vue"
import ImageMain from "@/components/publicComponents/ImageMain.vue"
...
const handleAsideChange = (image_class_id: any) => ImageMainRef.value.loadData(image_class_id)
...
const submit = () => {
    ...
    if (props.limit == 1) {
        ...
        value = props.preview ? [...(props as any).modelValue, ...urls] : [...urls]
        if (value.length > props.limit) {
            let limit = props.preview ? (props.limit - (props as any).modelValue.length) : props.limit
...
const removeImage = (url: any) => emit("update:modelValue", (props as any).modelValue.filter((u: any) => u != url))
