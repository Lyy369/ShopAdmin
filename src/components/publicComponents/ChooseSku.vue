...
import { getSkusList } from "@/api/suks"
...
function fn(res: any) {
    ...
    if (tableData.value.length > 0) {
        handleChangeActiveId((tableData.value[0] as any).id)
    }
}
const {
    loading,
...
function handleChangeActiveId(id: any) {
    ...
    if (item) {
        list.value = (item as any).default.split(",");
...
const submit = () => {
    if (typeof callbackFunction.value === "function") {
        (callbackFunction as any).value(form);
    }
        ...
        <el-container style="height:65vh;">
            <el-aside width="220px" class="image-aside">
                <div class="top">
                    ...
                    <div class="sku-list" :class="{ 'active': (activeId == (item as any).id) }"
                        v-for="(item, index) in tableData" :key="index" @click="handleChangeActiveId((item as any).id)">
                        {{ (item as any).name }}
                    </div>
