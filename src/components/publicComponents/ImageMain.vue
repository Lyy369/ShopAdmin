...
import UploadFile from "@/components/publicComponents/UploadFile.vue";
...
function getData(p: any = null) {
    ...
    getImageList(image_class_id.value, currentPage.value)
        .then(res => {
            ...
            list.value = (res as any).list.map((o: any) => {
                o.checked = false
                return o
            })
        })
...
const checkedImage = computed(() => list.value.filter(o => (o as any).checked))
            ...
            <el-row :gutter="10">
                <el-col :span="6" :offset="0" v-for="(item, index) in list" :key="index">
                    <el-card shadow="hover" class="relative mb-3" :body-style="{ 'padding': 0 }"
                        :class="{ 'border-blue-500': (item as any).checked }">
                        <el-image :src="(item as any).url" fit="cover" class="h-[150px]" style="width: 100%;"
                            :preview-src-list="[(item as any).url]" :initial-index="0"></el-image>
                        <div class="image-title">{{ (item as any).name }}</div>
                        <div class="flex items-center justify-center p-2">

                            <el-checkbox v-if="openChoose" v-model="(item as any).checked"
                                @change="handleChooseChange(item)" />

                            <el-button type="primary" size="small" text @click="handleEdit(item)">重命名</el-button>

                            <el-popconfirm title="是否删除图片？" confirmButtonText="确认" cancelButtonText="取消"
