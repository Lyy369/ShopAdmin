...
import ChooseImage from "@/components/publicComponents/ChooseImage.vue";
import Search from "@/components/publicComponents/Search.vue";
import SearchItem from "@/components/publicComponents/SearchItem.vue";
...
getCategoryList().then((res:any) => category_list.value = res)
...
<template>
      <div>
            ...
            <el-card shadow="never" class="border-0">
                        ...
                        <template #show>
                              <SearchItem label="商品分类">
                                    <el-select v-model="searchForm.category_id" placeholder="请选择商品分类" clearable>
                                          <el-option v-for="item in category_list" :key="(item as any).id" :label="(item as any).name"
                              ...
                              <template #default="{ row }">
                                    <div class="flex">
                                          ...
                                          <div class="flex-1">
                                                ...
                                                <p class="text-gray-400 text-xs mb-1">分类:{{ row.category ? row.category.name :
                                                      "未分类" }}</p>
                        ...
                        <el-table-column label="操作" align="center">
                              <template #default="scope">
                                    <div v-if="searchForm.tab != 'delete'">
                                          ...
                                          <el-button class="px-1"
                                                :type="scope.row.goods_banner.length == 0 ? 'danger' : 'primary'" size="small"
                                          ...
                                          <el-button class="px-1" :type="!scope.row.content ? 'danger' : 'primary'"
                                                size="small" text @click="handleSetGoodsContent(scope.row)"
                  ...
                  <FormDrawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit" confirm-text="提交">
                        <el-form :model="form" ref="formRef" :rules="rules" label-width="80px" :inline="false">
                              ...
                              <el-form-item label="商品分类" prop="category_id">
                                    <el-select v-model="form.category_id" placeholder="选择所属商品分类">
                                          <el-option v-for="item in category_list" :key="(item as any).id" :label="(item as any).name"
                                                :value="(item as any).id"></el-option>
                                    </el-select>
                              </el-form-item>
